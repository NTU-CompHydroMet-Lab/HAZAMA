...
def preprocess_data(filepath):
    ...
    # ----------------------------------------------
    emdat_derived = emdat_data[['DisNo.', 'ISO', 'Country', 'Location', 'Latitude', 'Longitude', 'Start Year', 'Start Month', 'Start Day', 'End Year', 'End Month', 'End Day', 'Admin Units', 'GADM Admin Units']].copy()
    ...
    emdat_derived['admin_list_raw_gadm'] = emdat_derived['GADM Admin Units'].apply(parse_admin_units_safe)
    emdat_derived['admin_list_structured'] = emdat_derived.apply(lambda row: reorganize_admin_data(row['admin_list_raw'], row['admin_list_raw_gadm']), axis=1)
...
def get_bbox_from_gee(row, gaul_dataset, gadm_dataset):
    ...
    adm2_code_gadm = row['adm2_code_gadm'] if pd.notna(row.get('adm2_code_gadm')) else None
    adm2_name_gadm = row['adm2_name_gadm'] if pd.notna(row.get('adm2_name_gadm')) else None
    adm1_list_gadm = row['adm1_name_list_gadm'] if isinstance(row.get('adm1_name_list_gadm'), list) and len(row.get('adm1_name_list_gadm')) > 0 else None
    ...
    if target_feature is None and adm2_name_gadm is not None and adm1_list_gadm is not None:
